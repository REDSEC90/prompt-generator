...
const variations_1 = require("../src/core/variations");
const retry_1 = require("../src/core/retry");
const templates_1 = require("../src/core/templates");
...
describe('formatLabel', () => {
    it('retorna rótulo legível para todos os formatos', () => {
        expect((0, templates_1.formatLabel)('markdown')).toBe('Markdown com seções H2 e bullet points');
        expect((0, templates_1.formatLabel)('json')).toBe('JSON válido');
        expect((0, templates_1.formatLabel)('table')).toBe('tabela Markdown');
        expect((0, templates_1.formatLabel)('numbered-list')).toBe('lista numerada');
        expect((0, templates_1.formatLabel)('prose')).toBe('texto corrido em parágrafos');
        expect((0, templates_1.formatLabel)('html')).toBe('HTML semântico');
        expect((0, templates_1.formatLabel)('code')).toBe('bloco de código com comentários');
    });
});
// ─── toneLabel ───────────────────────────────────────────────────────────────
describe('toneLabel', () => {
    it('retorna rótulo legível para todos os tons', () => {
        expect((0, templates_1.toneLabel)('formal')).toBe('formal e objetivo');
        expect((0, templates_1.toneLabel)('friendly')).toBe('amigável e acessível');
        expect((0, templates_1.toneLabel)('persuasive')).toBe('persuasivo, com senso de urgência');
        expect((0, templates_1.toneLabel)('didactic')).toBe('didático e progressivo');
        expect((0, templates_1.toneLabel)('journalistic')).toBe('jornalístico, direto');
        expect((0, templates_1.toneLabel)('technical')).toBe('técnico e preciso');
    });
    it('retorna o valor original para tom desconhecido', () => {
        expect((0, templates_1.toneLabel)('unknown')).toBe('unknown');
    });
});
// ─── TemplateEngine.validate ─────────────────────────────────────────────────
describe('TemplateEngine.validate', () => {
    const engine = new engine_1.TemplateEngine();
...
describe('TemplateEngine.fill', () => {
    ...
    it('lança erro para categoria desconhecida', () => {
        ...
        expect(() => engine.engine?.fill?.({ ...BASE, category: 'unknown' })).not.toThrow();
        // Teste direto via cast
        const spy = jest.spyOn(engine, 'validate').mockReturnValue({ valid: true, errors: [], warnings: [] });
        expect(() => engine.fill({ ...BASE, category: 'unknown' })).toThrow('Categoria desconhecida');
        spy.mockRestore();
    });
    it('gera prompt para todas as 8 categorias sem lançar erro', () => {
        const categories = ['summary', 'code', 'analysis', 'marketing', 'brainstorming', 'translation', 'qa', 'creative'];
...
describe('VariationGenerator.generate', () => {
    const gen = new variations_1.VariationGenerator();
    ...
    it('variação chainOfThought de category:code NÃO contém instrução CoT', () => {
        const codeConfig = { ...BASE, category: 'code', format: 'code', language: 'TypeScript' };
...
describe('parseArgs', () => {
    it('reconhece flags parciais como entrada CLI sem marcar config completa', () => {
        const parsed = (0, args_1.parseArgs)(['node', 'cli', '--theme', 'JWT']);
    ...
    it('respeita a flag --interactive', () => {
        const parsed = (0, args_1.parseArgs)(['node', 'cli', '--interactive', '--theme', 'JWT']);
...
describe('RetryManager', () => {
    it('retorna o resultado na primeira tentativa bem-sucedida', async () => {
        const rm = new retry_1.RetryManager(3, 0);
    ...
    it('retenta em erro 429 (rate limit)', async () => {
        const rm = new retry_1.RetryManager(2, 0);
        let calls = 0;
        await expect(rm.execute(async () => {
            calls++;
    ...
    it('lança após esgotar todas as tentativas', async () => {
        const rm = new retry_1.RetryManager(2, 0);
        let calls = 0;
        await expect(rm.execute(async () => { calls++; throw new Error('network'); })).rejects.toThrow('network');
        expect(calls).toBe(3);
    });
});
...
const fields_1 = require("../src/cli/fields");
const mockInput = prompts_1.input;
const mockSelect = prompts_1.select;
const mockConfirm = prompts_1.confirm;
describe('collectRequired', () => {
    ...
    it('retorna os 7 campos obrigatórios com os valores fornecidos', async () => {
        ...
        const result = await (0, fields_1.collectRequired)({});
    ...
    it('usa valores de initial como default', async () => {
        ...
        await (0, fields_1.collectRequired)({ theme: 'JWT', category: 'code' });
...
describe('collectOptional', () => {
    ...
    it('retorna language e limit quando preenchidos', async () => {
        ...
        const result = await (0, fields_1.collectOptional)({}, 'code');
    ...
    it('retorna fewShot quando confirmado', async () => {
        ...
        const result = await (0, fields_1.collectOptional)({}, 'analysis');
    ...
    it('retorna restrictions como array', async () => {
        ...
        const result = await (0, fields_1.collectOptional)({}, 'summary');
    ...
    it('adapta mensagem de language para category:translation', async () => {
        mockInput.mockResolvedValue('');
        mockConfirm.mockResolvedValueOnce(false);
        await (0, fields_1.collectOptional)({}, 'translation');
        const firstCall = mockInput.mock.calls[0][0];
